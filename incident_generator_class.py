...
class JsonToIncident:
	def __init__(self):
		...
		self.__endDate = date(date.today().year, date.today().month, monthrange(date.today().month, date.today().year())[1])
...
def generateRangeSeperateMonths(cache, begin_month, begin_year, end_month, end_year):
	...
	i_month = begin_month
	i_year = begin_year
	days_in_month = monthrange(i_year, i_month)[1];
	
	while (i_month < end_month and i <= end_year):
	cache.setCacheByDate(date(i_year, i_month, 1), date(i_year, i_month, days_in_month))
	jsonFname = ItoJ.createJson(cache.incidents)
	fname = "incidents_" + str(i_month) + "_" + str(i_year) + ".json"
...
def generateUpdatedJsonByMonth(cache, month, year):
	...
	#db_initial.runDB()   
	cache.setCacheByDate(date(year, month, 1), date(year, month, monthrange(year, month)[1]))
...
def run():
	cache = IncidentCache()
	cache.setJsonFiles({(12,2016): "./pdfs/DEC_16.json", (11,2016): "./pdfs/NOV_16.json",(10,2016): "./pdfs/OCT_16.json",(9,2016): "./pdfs/SEP_16.json",(8,2016): "./pdfs/AUG_16.json",(7,2016): "./pdfs/JUL_16.json",(6,2016): "./pdfs/JUN_16.json",(5,2016): "./pdfs/MAY_16.json",(4,2016): "./pdfs/APR_16.json",(3,2016): "./pdfs/MAR_16.json",(2,2016): "./pdfs/FEB_16.json",(1,2016): "./pdfs/JAN_16.json"})
		
	generateRangeSeperateMonths(cache, 11, 2016, 12, 2016)
