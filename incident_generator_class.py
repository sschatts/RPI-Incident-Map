...
class IncidentToJson():
	...
	def createJson(self, incidents, fname, location_group):
		...
		if (location_group != "all"):
			...
			fname = fname_spl[0] + "_" + location_group + "." + fname_spl[1]
		with open("./pdfs/{fn}".format(fn = fname), "w") as jsonFile:
			for incid in incidents:
				...
				if (incid.location.locationGroup == location_group or location_group == "all"):
...
def generateUpdatedJsonByMonth(cache, month, year):
	...
	l_groups = ["North", "East", "South", "West", "Off-Campus", "Academic", "all"]
	
	for loc in l_groups:
		jsonFname = ItoJ.createJson(cache.incidents, fname, loc)
	#testJsonSuccessfullyCreated(jsonFname)
