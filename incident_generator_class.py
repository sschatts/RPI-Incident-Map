...
class IncidentData:
    ...
    def getCat(self, description):
        ...
        while (desc_category != "" and (i < len(self.__categories) ) ):
            if (description.upper() in self.__categories[i].upper()):
                desc_category = desc_category + self.__categories[i]
...
class JsonToIncident:
    ...
    def setDateRangeFullAvailable(self):
        ...
        min_date = (date.today().month, date.today().year)

        for d in self.__jsonFiles:
            if (d[1] < min_date[1]):
                min_date = d
            elif (d[1] == min_date[1] and d[0] < min_date[0]):
                min_date = d

        self.__beginDate = Date(min_date[1], min_date[0], 1)
        max_date = min_date

        for d in self.__jsonFiles:
            if (d[1] > max_date[1]):
                max_date = d
            elif (d[1] == max_date[1] and d[0] > miax_date[0]):
                max_date = d

        self.__endDate = max_date
